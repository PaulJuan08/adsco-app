...
<style>
    /* ONLY fix for sidebar layout conflict - absolutely necessary */
...
<div class="form-container">
    <!-- Card Header with gradient - using exact classes from course-show.css -->
    ...
    <div class="card-body">
        ...
        <div style="margin-bottom: 2rem; padding: 1.5rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 16px; color: white; box-shadow: 0 10px 25px rgba(102, 126, 234, 0.3);">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; text-align: center;">
                <div>
                    <div style="font-size: 1.5rem; font-weight: 700;">{{ $courseProgress['completed'] }}</div>
                    <div style="font-size: 0.875rem; opacity: 0.9;">Completed</div>
                </div>
                <div>
                    <div style="font-size: 1.5rem; font-weight: 700;">{{ $courseProgress['remaining'] }}</div>
                    <div style="font-size: 0.875rem; opacity: 0.9;">Remaining</div>
                </div>
                <div>
                    <div style="font-size: 1.5rem; font-weight: 700;">{{ $courseProgress['total'] }}</div>
                    <div style="font-size: 0.875rem; opacity: 0.9;">Total Topics</div>
                </div>
            </div>
        </div>
        @endif
        
        <!-- Details Grid - using exact classes from course-show.css -->
        <div class="details-grid">
            ...
            <div class="detail-section">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ ucfirst($course->level) }}</span>
                </div>
                @endif
        ...
        if (searchInput && topicCards.length > 0) {
            searchInput.addEventListener('input', function(e) {
                ...
                topicCards.forEach(card => {
                    const topicTitle = card.dataset.title || '';
