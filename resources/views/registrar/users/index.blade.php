...
@section('title', 'User Management - Registrar Dashboard')
...
<div class="dashboard-container">
            ...
            <div class="user-greeting">
                ...
                <div class="greeting-text">
                    ...
                    <p class="welcome-subtitle">
                        <i class="fas fa-users"></i> Manage teachers and students
                        @if($stats['pending'] > 0)
    ...
    <div class="stats-grid stats-grid-compact">
        <a href="{{ route('registrar.users.index') }}" class="stat-card stat-card-primary clickable-card">
        ...
        <a href="{{ route('registrar.users.index') }}?status=pending" class="stat-card stat-card-warning clickable-card">
            <div class="stat-header">
                <div>
        ...
        <a href="{{ route('registrar.users.index') }}?role=3" class="stat-card stat-card-success clickable-card">
            <div class="stat-header">
                <div>
        ...
        <a href="{{ route('registrar.users.index') }}?role=4" class="stat-card stat-card-info clickable-card">
            <div class="stat-header">
                <div>
    ...
    <div class="content-grid">
                ...
                <div class="card-body">
                    ...
                    <div class="filters-section">
                        <div class="filter-buttons">
                            <a href="{{ route('registrar.users.index') }}" class="filter-btn {{ !request()->has('role') && !request()->has('status') ? 'active' : '' }}">
                                All Users
                            </a>
                            <a href="{{ route('registrar.users.index') }}?status=pending" class="filter-btn {{ request('status') == 'pending' ? 'active' : '' }}">
                                <i class="fas fa-clock"></i> Pending
                            </a>
                            <a href="{{ route('registrar.users.index') }}?role=3" class="filter-btn {{ request('role') == '3' ? 'active' : '' }}">
                                <i class="fas fa-chalkboard-teacher"></i> Teachers
                            </a>
                            <a href="{{ route('registrar.users.index') }}?role=4" class="filter-btn {{ request('role') == '4' ? 'active' : '' }}">
                                ...
                                <a href="{{ route('registrar.users.show', Crypt::encrypt($user->id)) }}" class="item-clickable-area">
                                    ...
                                    <div class="item-info">
                                        ...
                                        <div class="item-meta">
                                            ...
                                            @if($user->role == 3 && $user->employee_id)
                                                <span class="item-badge badge-secondary">
                                                    <i class="fas fa-id-card"></i> {{ $user->employee_id }}
                                                </span>
                                            @elseif($user->role == 4 && $user->student_id)
                                                <span class="item-badge badge-secondary">
        ...
        <div class="right-column">
            ...
            <div class="dashboard-card">
                ...
                <div class="card-body">
                    <div class="quick-actions-grid">
                        ...
                        <a href="{{ route('registrar.users.index') }}?status=pending" class="action-card action-success">
                            <div class="action-icon">
            ...
            <div class="dashboard-card">
                ...
                <div class="card-body">
                    <div class="items-list">
                        <a href="{{ route('registrar.users.index') }}?role=3" class="list-item clickable-item">
                            <div class="item-avatar" style="border-radius: var(--radius); background: linear-gradient(135deg, var(--success-light), var(--success));">
                                <i class="fas fa-chalkboard-teacher"></i>
                            </div>
                            <div class="item-info">
                                <div class="item-name">Total Teachers</div>
                                <div class="item-details">{{ $stats['approved_teachers'] }} approved</div>
                            </div>
                            <div class="stat-number" style="font-size: 1.5rem;">{{ $stats['teachers'] }}</div>
                        </a>
                        
                        <a href="{{ route('registrar.users.index') }}?role=4" class="list-item clickable-item">
                            ...
                            <div class="item-info">
                                ...
                                <div class="item-details">{{ $stats['approved_students'] }} approved</div>
                            </div>
                            <div class="stat-number" style="font-size: 1.5rem;">{{ $stats['students'] }}</div>
                        </a>
                        
                        <a href="{{ route('registrar.users.index') }}?status=pending" class="list-item clickable-item">
    ...
    <div id="print-content" class="print-content">
        <div style="padding: 20px; font-family: Arial, sans-serif;">
            <div style="text-align: center; margin-bottom: 20px;">
                <h1 style="color: #4f46e5; margin-bottom: 5px;">Registrar User Management Report</h1>
            ...
            <div style="margin-bottom: 30px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 20px;">
                    ...
                    <div style="background: #fef3c7; padding: 15px; border-radius: 8px; border: 1px solid #fde68a;">
                        <div style="font-size: 24px; font-weight: bold; color: #d97706; margin-bottom: 5px;">{{ $stats['pending'] }}</div>
                    ...
                    <div style="background: #eef2ff; padding: 15px; border-radius: 8px; border: 1px solid #c7d2fe;">
                        <div style="font-size: 24px; font-weight: bold; color: #4f46e5; margin-bottom: 5px;">{{ $stats['this_month'] }}</div>
                        <div style="font-size: 14px; color: #4f46e5;">New This Month</div>
                    </div>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('print-report')?.addEventListener('click', function() {
            ...
            printWindow.document.write(`
                ...
                <html>
                <head>
                    <title>Registrar User Management Report</title>
                    <style>
                        @media print {
                            @page {
                                size: landscape;
        ...
        document.getElementById('export-csv')?.addEventListener('click', function() {
            ...
            // Add headers
            const headers = ['Name', 'Email', 'Role', 'ID', 'Status', 'Join Date', 'User ID'];
            ...
            userItems.forEach(item => {
                ...
                // ID (Employee ID or Student ID)
                const idBadge = item.querySelectorAll('.item-badge')[1];
                if (idBadge && !idBadge.classList.contains('badge-secondary')) {
                    const idText = idBadge.textContent.trim().split(' ').slice(1).join(' ');
                    cells.push(`"${idText}"`);
                } else {
                    cells.push('""');
                }
                ...
                const timeSpan = item.querySelector('.item-badge.badge-secondary:last-child');
                if (timeSpan) {
                    const timeText = timeSpan.textContent.trim().split(' ').slice(1).join(' ');
                    cells.push(`"${timeText}"`);
                }
