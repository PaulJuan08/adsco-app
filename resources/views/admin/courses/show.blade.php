        ...
        <div class="card" style="margin-bottom: 1.5rem;">
            ...
            <div style="padding: 1rem;">
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <a href="#" 
                       style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 6px; text-decoration: none; color: #374151; transition: all 0.2s;">
                        <div style="width: 32px; height: 32px; background: #f59e0b; border-radius: 6px; display: flex; align-items: center; justify-content: center; color: white;">
                            <i class="fas fa-eye-slash"></i>
                        </div>
                        <div>
                            <div style="font-weight: 500; font-size: 0.875rem;">Unpublish Course</div>
                            <div style="font-size: 0.75rem; color: #6b7280;">Make course private</div>
                        </div>
                        <i class="fas fa-chevron-right" style="margin-left: auto; color: #9ca3af;"></i>
                    </a>
                    @else
                    <a href="#" 
                       style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 6px; text-decoration: none; color: #374151; transition: all 0.2s;">
                        ...
                        <div>
                            <div style="font-weight: 500; font-size: 0.875rem;">Publish Course</div>
                            <div style="font-size: 0.75rem; color: #6b7280;">Make course public</div>
                        </div>
                        <i class="fas fa-chevron-right" style="margin-left: auto; color: #9ca3af;"></i>
                    </a>
                    @endif
...
<div class="modal-overlay" id="addTopicModal">
    <div class="modal-container">
        <div class="modal-header">
            <div class="modal-title">Adasdasdd Topics to Course</div>
...
function loadAvailableTopics() {
    console.log('DEBUG: Loading available topics for course ID:', {{ $course->id }});
    console.log('Encrypted ID:', encryptedCourseId);
    ...
    .then(data => {
        ...
        if (Array.isArray(data)) {
            console.log('Number of topics:', data.length);
...
function renderAvailableTopics(topics) {
    ...
    container.innerHTML = topics.map(topic => {
        ...
        return `
            <div class="topic-item ${selectedTopics.includes(topic.id) ? 'selected' : ''}" 
...
function toggleTopic(topicId) {
    ...
    items.forEach(item => {
        const titleElement = item.querySelector('.topic-item-title');
        if (titleElement) {
            item.classList.toggle('selected', selectedTopics.includes(topicId));
        }
    });
}
