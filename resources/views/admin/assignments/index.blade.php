...
<div class="dashboard-container">
        ...
        <div class="header-content">
            <div class="user-greeting">
                ...
                <div class="greeting-text">
                    ...
                    <p class="welcome-subtitle"><i class="fas fa-file-alt"></i> Manage assignments and student submissions</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="stats-grid-compact">
        ...
        <div class="stat-card stat-card-info clickable-card" onclick="window.location.href='{{ route('admin.todo.progress', ['type' => 'assignment']) }}'">
            <div class="stat-header">
    ...
    <div class="header-actions" style="justify-content:space-between;margin-bottom:1.5rem;">
        <form method="GET" action="{{ route('admin.assignments.index') }}" style="display:flex;gap:.5rem;">
            ...
            @if($search)
            <a href="{{ route('admin.assignments.index') }}" class="btn-xs btn-xs-outline"><i class="fas fa-times"></i> Clear</a>
            @endif
        </form>
        <div style="display:flex;gap:.5rem;align-items:center;">
            ...
            <a href="{{ route('admin.todo.progress', ['type' => 'assignment']) }}" class="btn-xs btn-xs-outline">
    ...
    <div class="todo-grid">
        ...
        @endphp
        <div class="todo-card" onclick="window.location.href='{{ route('admin.todo.assignment.show', $encId) }}'">
                    ...
                    <div class="creator-info">
                        ...
                        <span class="creator-name">
                            @if($assignment->creator)
                                ...
                                <span class="creator-badge" style="background:{{ $assignment->creator->role == 1 ? '#f59e0b' : ($assignment->creator->role == 3 ? '#48bb78' : '#718096') }};color:#fff;padding:2px 8px;border-radius:12px;font-size:10px;margin-left:6px;">
            ...
            <div class="students-preview">
                ...
                <a href="{{ route('admin.todo.progress', ['type' => 'assignment', 'item_id' => $assignment->id]) }}" class="view-all-link" onclick="event.stopPropagation()">
            ...
            <div class="card-footer-actions" onclick="event.stopPropagation()">
                <form method="POST" action="{{ route('admin.assignments.publish', $encId) }}" style="margin:0;">
                    @csrf @method('PATCH')
                ...
                <div class="action-dropdown-wrapper">
                    ...
                    <div class="action-dropdown-menu">
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); openCrudModal('{{ route('admin.todo.assignment.access.modal', $encId) }}', 'Students — {{ addslashes($assignment->title) }}', '960px')" class="dropdown-item">
                            <i class="fas fa-users"></i> View Students
                        </button>
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); openCrudModal('{{ route('admin.todo.assignment.access.modal', $encId) }}', 'Grant Access — {{ addslashes($assignment->title) }}', '960px')" class="dropdown-item">
                            <i class="fas fa-key"></i> Grant Access
                        </button>
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); openCrudModal('{{ route('admin.assignments.edit', $encId) }}', 'Edit Assignment')" class="dropdown-item">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <a href="{{ route('admin.todo.assignment.show', $encId) }}" class="dropdown-item" onclick="closeAllDropdowns()">
                            <i class="fas fa-eye"></i> View
                        </a>
                        <a href="{{ route('admin.todo.progress', ['type' => 'assignment', 'item_id' => $assignment->id]) }}" class="dropdown-item" onclick="closeAllDropdowns()">
                            <i class="fas fa-chart-bar"></i> Progress
                        </a>
    ...
    <div class="empty-todo">
        ...
        <div style="display:flex;gap:.75rem;justify-content:center;">
            ...
            @if($search)
            <a href="{{ route('admin.assignments.index') }}" class="btn-xs btn-xs-outline"><i class="fas fa-times"></i> Clear Search</a>
            @endif
        </div>
    </div>
    @endif
