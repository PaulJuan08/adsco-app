...
<div class="dashboard-container">
        ...
        <div class="header-content">
            <div class="user-greeting">
                ...
                <div class="greeting-text">
                    ...
                    <p class="welcome-subtitle"><i class="fas fa-brain"></i> Manage quizzes and student access</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="stats-grid stats-grid-compact">
        ...
        <div class="stat-card stat-card-info clickable-card" onclick="window.location.href='{{ route('admin.todo.progress', ['type' => 'quiz']) }}'">
            <div class="stat-header">
    ...
    <div class="header-actions" style="justify-content:space-between;margin-bottom:1.5rem;">
        <form method="GET" action="{{ route('admin.quizzes.index') }}" style="display:flex;gap:.5rem;">
            ...
            @if($search)
            <a href="{{ route('admin.quizzes.index') }}" class="btn-xs btn-xs-outline"><i class="fas fa-times"></i> Clear</a>
            @endif
        </form>
        <div style="display:flex;gap:.5rem;align-items:center;">
            ...
            <a href="{{ route('admin.todo.progress', ['type' => 'quiz']) }}" class="btn-xs btn-xs-outline">
                <i class="fas fa-chart-bar"></i> Progress
            </a>
            <a href="{{ route('admin.todo.index') }}" class="btn-xs btn-xs-outline">
    ...
    <div class="todo-grid">
        ...
        @endphp
        <div class="todo-card" onclick="window.location.href='{{ route('admin.todo.quiz.show', $encId) }}'">
                    ...
                    <div class="creator-info">
                        ...
                        <span class="creator-name">
                            @if($quiz->creator)
                                ...
                                <span class="creator-badge" style="background:{{ $quiz->creator->role == 1 ? '#f59e0b' : ($quiz->creator->role == 3 ? '#48bb78' : '#718096') }};color:#fff;padding:2px 8px;border-radius:12px;font-size:10px;margin-left:6px;">
            ...
            <div class="students-preview">
                ...
                @if($attemptCount > 5)
                <a href="{{ route('admin.todo.progress', ['type' => 'quiz', 'item_id' => $quiz->id]) }}" class="view-all-link" onclick="event.stopPropagation()">
            ...
            @endif
                ...
                <form method="POST" action="{{ route('admin.quizzes.toggle-publish', $encId) }}" style="margin:0;">
                    @csrf
                ...
                <div class="action-dropdown-wrapper">
                    ...
                    <div class="action-dropdown-menu">
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); openCrudModal('{{ route('admin.todo.quiz.access.modal', $encId) }}', 'Students — {{ addslashes($quiz->title) }}', '960px')" class="dropdown-item">
                            <i class="fas fa-users"></i> View Students
                        </button>
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); openCrudModal('{{ route('admin.todo.quiz.access.modal', $encId) }}', 'Grant Access — {{ addslashes($quiz->title) }}', '960px')" class="dropdown-item">
                            <i class="fas fa-key"></i> Grant Access
                        </button>
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); openCrudModal('{{ route('admin.quizzes.edit', $encId) }}', 'Edit Quiz')" class="dropdown-item">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <a href="{{ route('admin.todo.quiz.show', $encId) }}" class="dropdown-item" onclick="closeAllDropdowns()">
                            <i class="fas fa-eye"></i> View
                        </a>
                        <a href="{{ route('admin.todo.progress', ['type' => 'quiz', 'item_id' => $quiz->id]) }}" class="dropdown-item" onclick="closeAllDropdowns()">
                            <i class="fas fa-chart-bar"></i> Progress
                        </a>
                        <div class="dropdown-divider"></div>
                        <button onclick="event.stopPropagation(); closeAllDropdowns(); confirmDeleteItem('{{ route('admin.quizzes.destroy', $encId) }}', '{{ addslashes($quiz->title) }}')" class="dropdown-item text-danger">
    ...
    <div class="empty-todo">
        ...
        <div style="display:flex;gap:.75rem;justify-content:center;">
            <button onclick="openCrudModal('{{ route('admin.quizzes.create') }}', 'New Quiz')" class="btn-xs btn-xs-primary"><i class="fas fa-plus-circle"></i> New Quiz</button>
            @if($search)
            <a href="{{ route('admin.quizzes.index') }}" class="btn-xs btn-xs-outline"><i class="fas fa-times"></i> Clear Search</a>
            @endif
        </div>
    </div>
    @endif
