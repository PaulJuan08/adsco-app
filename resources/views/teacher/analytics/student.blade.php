...
<div class="analytics-page">
    ...
    <div class="analytics-header no-print">
        <div class="analytics-header-left">
            <a href="{{ route('teacher.analytics.index', ['tab' => 'students']) }}"
            ...
            <div>
                ...
                <div class="analytics-header-sub">Performance in your courses</div>
            </div>
        </div>
    ...
    <div class="analytics-charts-grid">
        ...
        <div class="chart-card-a chart-full">
            ...
            <div class="chart-container" style="min-height:{{ max(180, $courseTopicProgress->count() * 38 + 40) }}px">
                ...
                @else
                <div class="analytics-empty"><i class="fas fa-book"></i><p>No courses</p></div>
                @endif
            </div>
        </div>
            ...
            <table class="analytics-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge-a badge-{{ $sub->status }}">{{ ucfirst($sub->status) }}</span></td>
...
new Chart(document.getElementById('quizTrendChart'), {
    ...
    data: {
        ...
        datasets: [{ label:'Score %', data:{!! json_encode($quizTrend->pluck('score')) !!}, borderColor:P.brown, backgroundColor:'rgba(85,43,32,0.08)', tension:0.4, fill:true, pointBackgroundColor:{!! json_encode($quizTrend->map(fn($t)=>$t['score']>=75?'#48bb78':($t['score']>=50?'#ddb238':'#f56565'))) !!}, pointRadius:6, pointHoverRadius:8 }]
    },
    options: { responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false},tooltip:{...tipOpts,callbacks:{afterLabel:ctx=>{ const q={!! json_encode($quizTrend->pluck('quiz')) !!}; return q[ctx.dataIndex]??''; }}}}, scales:{y:{beginAtZero:true,max:100,ticks:{callback:v=>v+'%'}}} }
});
@endif
...
new Chart(document.getElementById('courseProgressChart'), {
    ...
    data: {
        ...
        datasets: [{ label:'Completion %', data:{!! json_encode($courseTopicProgress->pluck('pct')) !!}, backgroundColor:{!! json_encode($courseTopicProgress->map(fn($c)=>$c['pct']>=70?'#48bb78':($c['pct']>=40?'#ddb238':'#f56565'))->values()) !!}, borderRadius:5 }]
    },
...
new Chart(document.getElementById('avgScoreChart'), {
    ...
    data: {
        ...
        datasets: [{ label:'Avg Score %', data:{!! json_encode($avgScorePerCourse->values()) !!}, backgroundColor:P.gold, borderRadius:5 }]
    },
    options: { indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false},tooltip:tipOpts}, scales:{x:{beginAtZero:true,max:100,ticks:{callback:v=>v+'%'}}} }
});
@endif
