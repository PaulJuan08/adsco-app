...
@section('title', 'Enrollment Management - Teacher Dashboard')
            ...
            <div class="user-greeting">
                ...
                <div class="greeting-text">
                    ...
                    <p class="welcome-subtitle">
                        <i class="fas fa-user-graduate"></i> Manage your course enrollments
                    </p>
                </div>
            </div>
    ...
    <div class="enrollment-container">
        <div class="courses-grid">
            ...
            <div class="empty-state" style="grid-column: 1/-1;">
                ...
                <p>You don't have any courses yet. Courses are assigned to you by an administrator.</p>
...
<script>
    ...
    async function loadEnrolledStudentIds(encryptedCourseId) {
        try {
            const response = await fetch(`/teacher/enrollments/course/${encodeURIComponent(encryptedCourseId)}/student-ids`);
    ...
    function modalLoadPrograms(collegeId) {
        ...
        fetch(`/teacher/enrollments/programs/${collegeId}`)
    ...
    function modalLoadStudents(page = 1) {
        ...
        let url = `/teacher/enrollments/students?page=${page}&course_id=${encodeURIComponent(currentEncryptedCourseId)}`;
    ...
    function modalUpdateSelectedCount() {
        ...
        enrollButton.disabled = !(count > 0 && currentEncryptedCourseId);
    }

    function modalClearSelections() {
        modalSelectedStudentIds = [];
        modalUpdateSelectedCount();
        document.querySelectorAll('#modalStudentsList .student-checkbox input[type="checkbox"]:not(:disabled)').forEach(cb => {
            cb.checked = false;
        });
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .catch(error => {
                    ...
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to enroll students. Please try again.' });
                })
                .finally(() => {
    ...
    function updateCourseCount(encryptedCourseId, addedCount) {
        ...
        if (courseCard) {
            ...
            if (statValue) {
                statValue.textContent = parseInt(statValue.textContent) + addedCount;
            }
            if (viewButton) {
                const match = viewButton.textContent.match(/\((\d+)\)/);
                if (match) {
                    viewButton.innerHTML = `<i class="fas fa-users"></i> View (${parseInt(match[1]) + addedCount})`;
                }
            }
        }
    }
    ...
    function loadEnrolledStudents(encryptedCourseId) {
        ...
        fetch(`/teacher/enrollments/course/${encodeURIComponent(encryptedCourseId)}/students`)
            .then(response => response.json())
            .then(students => {
    ...
    function searchEnrolledStudents() {
        ...
        viewFilteredStudents = !searchTerm
            ? viewAllEnrolledStudents
            : viewAllEnrolledStudents.filter(student =>
    ...
    function removeStudentFromView(studentId, studentName, encryptedCourseId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('{{ route("teacher.enrollments.remove") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                .catch(error => {
                    console.error('Error:', error);
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to remove student. Please try again.' });
                });
            }
        });
    }
    ...
    function updateCourseCountAfterRemoval(encryptedCourseId) {
        ...
        if (courseCard) {
            ...
            if (statValue) {
                statValue.textContent = Math.max(0, parseInt(statValue.textContent) - 1);
            }
            if (viewButton) {
                const match = viewButton.textContent.match(/\((\d+)\)/);
                if (match) {
                    viewButton.innerHTML = `<i class="fas fa-users"></i> View (${Math.max(0, parseInt(match[1]) - 1)})`;
                }
            }
        }
    }
    ...
    document.addEventListener('click', function (event) {
        if (event.target === document.getElementById('enrollModal'))      closeEnrollModal();
        if (event.target === document.getElementById('viewStudentsModal')) closeViewStudentsModal();
    });
