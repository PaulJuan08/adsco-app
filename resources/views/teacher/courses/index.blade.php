...
@section('title', 'Courses - Teacher Dashboard')
...
<div class="top-header">
    <div class="greeting">
        <h1>My Courses</h1>
        <p>Manage and organize your teaching courses</p>
    </div>
    <div class="user-info">
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        <div class="stat-header">
            <div>
                @php
                    $totalStudents = $courses->sum('enrollments_count') ?? 0;
                @endphp
    ...
    <div class="stat-card">
        <div class="stat-header">
            <div>
                @php
                    $averageStudents = $courses->count() > 0 ? round($totalStudents / $courses->count(), 1) : 0;
                @endphp
                <div class="stat-number">{{ $averageStudents }}</div>
...
<div class="content-grid">
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title">My Teaching Courses</div>
            <div class="d-flex gap-2 align-items-center">
                ...
                <a href="{{ route('teacher.courses.create') }}" class="view-all" style="display: flex; align-items: center; gap: 6px;">
        ...
        <div class="empty-state">
            ...
            <p style="color: var(--secondary); margin-bottom: 24px; max-width: 400px; margin-left: auto; margin-right: auto;">
                You haven't created any courses. Start building your curriculum by adding your first course.
            </p>
            ...
            <div style="margin-top: 20px; color: var(--secondary); font-size: 0.875rem;">
                ...
                Courses you create will be available for student enrollment
            </div>
        </div>
        @else
        <!-- Courses List -->
                    ...
                    <tr style="border-bottom: 1px solid var(--border);">
                        <td style="padding: 16px;">
                            <div style="display: flex; align-items: center; gap: 12px;">
                                ...
                                <div>
                                    <div class="course-name">{{ $course->title ?? $course->course_name }}</div>
                                    <div style="font-size: 0.875rem; color: var(--secondary); margin-top: 2px;">
                        ...
                        <td style="padding: 16px;">
                            <div style="font-weight: 600; color: var(--dark);">{{ $course->enrollments_count ?? 0 }}</div>
                        ...
                        <td style="padding: 16px;">
                            <div style="display: flex; gap: 8px;">
                                <a href="{{ route('teacher.courses.show', $course->id) }}" title="View" style="padding: 8px; background: #e0e7ff; color: var(--primary); border-radius: 6px; text-decoration: none;">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('teacher.courses.edit', $course->id) }}" title="Edit" style="padding: 8px; background: #f3f4f6; color: var(--secondary); border-radius: 6px; text-decoration: none;">
    ...
    <div>
        <div class="card" style="margin-bottom: 1.5rem;">
            ...
            <div style="padding: 0.5rem;">
                <a href="{{ route('teacher.courses.create') }}" style="display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 8px; text-decoration: none; color: var(--dark); transition: background 0.3s;">
        ...
        <div class="card">
            ...
            <div style="padding: 0.5rem;">
                ...
                <div style="padding: 12px; border-bottom: 1px solid var(--border);">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                        <span style="color: var(--secondary); font-size: 0.875rem;">Total Students</span>
                        <span style="font-weight: 600;">{{ $totalStudents }}</span>
                    </div>
                </div>
                <div style="padding: 12px; border-bottom: 1px solid var(--border);">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                ...
                <div style="padding: 12px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                        ...
                        <span style="font-weight: 600;">{{ $averageStudents }}</span>
                    </div>
                </div>
            </div>
        </div>
...
<div id="print-content" style="display: none;">
    <div style="padding: 20px; font-family: Arial, sans-serif;">
        <div style="text-align: center; margin-bottom: 20px;">
            <h1 style="color: #4f46e5; margin-bottom: 5px;">{{ Auth::user()->f_name }}'s Teaching Courses Report</h1>
        ...
        <div style="margin-bottom: 30px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 20px;">
                ...
                <div style="background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #e5e7eb;">
                    <div style="font-size: 24px; font-weight: bold; color: #059669; margin-bottom: 5px;">{{ $totalStudents }}</div>
                    <div style="font-size: 14px; color: #6b7280;">Total Students</div>
                </div>
                <div style="background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #e5e7eb;">
                    <div style="font-size: 24px; font-weight: bold; color: #0369a1; margin-bottom: 5px;">{{ $averageStudents }}</div>
                    <div style="font-size: 14px; color: #6b7280;">Avg Students per Course</div>
                </div>
            </div>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
            <thead>
                <tr style="background: #f3f4f6;">
                    ...
                    <th style="padding: 12px; border: 1px solid #e5e7eb; text-align: left; font-weight: bold; color: #374151;">Credits</th>
            ...
            <tbody>
                @foreach($courses as $course)
                <tr>
                    <td style="padding: 12px; border: 1px solid #e5e7eb;">{{ $course->title ?? $course->course_name }}</td>
                    <td style="padding: 12px; border: 1px solid #e5e7eb;">{{ $course->course_code }}</td>
                    <td style="padding: 12px; border: 1px solid #e5e7eb;">{{ $course->credits }}</td>
                    <td style="padding: 12px; border: 1px solid #e5e7eb; text-align: center;">{{ $course->enrollments_count ?? 0 }}</td>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center;">
            <p style="color: #6b7280; font-size: 14px;">
                Teacher: {{ Auth::user()->f_name }} {{ Auth::user()->l_name }} | 
...
<script>
    ...
    document.getElementById('print-report')?.addEventListener('click', function() {
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>My Teaching Courses Report</title>
                <style>
                    @media print {
                        @page {
                            size: landscape;
    ...
    document.getElementById('export-csv')?.addEventListener('click', function() {
        ...
        table.querySelectorAll('tbody tr').forEach(row => {
            ...
            // Description
            const descriptionDiv = columns[2].querySelector('.course-desc');
            cells.push(`"${descriptionDiv ? descriptionDiv.textContent.trim() : ''}"`);
