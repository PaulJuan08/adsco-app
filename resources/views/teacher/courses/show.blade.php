            ...
            <div class="two-column-layout">
                ...
                <div class="sidebar-column">
                    ...
                    <div class="sidebar-card">
                        ...
                        <button onclick="openCrudModal('{{ route('teacher.courses.edit', $encryptedId) }}', 'Edit Course')" class="quick-action-link" style="border:none;cursor:pointer;width:100%;background:transparent;">
...
<form id="publish-form" method="POST" action="{{ route('teacher.courses.publish', $encryptedId) }}" style="display:none;">
    @csrf @method('PATCH')
</form>
<form id="delete-form" method="POST" action="{{ route('teacher.courses.destroy', $encryptedId) }}" style="display:none;">
    @csrf @method('DELETE')
</form>
@endsection
...
<script>
const ROUTES = {
    availableTopics : '{{ route("teacher.courses.available-topics", $encryptedId) }}',
    addTopic        : '{{ route("teacher.courses.add-topic",        $encryptedId) }}',
    addTopics       : '{{ route("teacher.courses.add-topics",       $encryptedId) }}',
    removeTopic     : '{{ route("teacher.courses.remove-topic",     $encryptedId) }}',
    accessModal     : '{{ route("teacher.courses.access.modal",     $encryptedId) }}',
    publishTopic    : '{{ url("teacher/topics") }}',
};
...
function initializeModalScripts() {
    ...
    if (collegeFilter) {
        ...
        fresh.addEventListener('change', function() {
            ...
            fetch(`/teacher/colleges/${this.value}/programs`).then(r => r.json()).then(list => list.forEach(p => {
...
function toggleTopicPublish(topicId, publish, btn) {
    ...
    if (!encryptedTopicId) { 
        showNotification('Cannot toggle — please refresh the page.', 'warning'); 
        return; 
    }
    ...
    Swal.fire({
        ...
        // Construct URL
        const endpoint = `/teacher/topics/${encryptedTopicId}/publish`;
        ...
        .then(data => {
            if (data.success) {
                ...
                showNotification(data.message || `Topic ${publish ? 'published' : 'unpublished'} successfully!`, 'success');
...
function renderAvailableTopics(topics) {
    ...
    if (!Array.isArray(topics) || topics.length === 0) {
        container.innerHTML = `
            ...
            <button onclick="openCrudModal('{{ route('teacher.topics.create') }}', 'New Topic')" class="btn-sm btn-sm-primary" style="margin-top:1rem;border:none;cursor:pointer;">
