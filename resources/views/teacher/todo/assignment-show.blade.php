            ...
            <div class="two-column-layout">
                ...
                <div class="sidebar-column">
                        ...
                        <div class="info-row-sm">
                            ...
                            <span class="val">
                                @if($assignment->creator)
                                    ...
                                    @if($assignment->creator->role == 1)<span style="color:#718096;">(Admin)</span>@endif
                                @else
                    ...
                    <div class="sidebar-card">
                        ...
                        <button onclick="openCrudModal('{{ route('teacher.assignments.edit', $encryptedId) }}', 'Edit Assignment')" class="quick-action-link" style="border:none;cursor:pointer;width:100%;background:transparent;">
...
<form id="publish-form" method="POST" action="{{ route('teacher.assignments.publish', $encryptedId) }}" style="display:none;">@csrf @method('PATCH')</form>
...
$submissionsJson = $submissions->map(function($s) {
    return [
        ...
        'grade_url'       => route('teacher.todo.submission.grade', $s->id),
    ];
})->values()->toArray();
@endphp
...
function openGradeModal(id) {
    ...
    answerEl.style.color = sub.answer_text ? '#2d3748' : '#a0aec0';
    ...
    if (sub.status === 'graded' && sub.score !== null) {
        ...
        fbEl.style.color = sub.feedback ? '#4a5568' : '#a0aec0';
...
function initializeModalScripts() {
    ...
    document.querySelectorAll('.student-checkbox').forEach(cb => {
        cb.addEventListener('change', function() {
            if (countDisplay) countDisplay.textContent = document.querySelectorAll('.student-checkbox:checked').length + ' selected';
        });
    });
    document.querySelectorAll('.toggle-access').forEach(toggle => {
        toggle.addEventListener('change', function() {
            const self = this, checked = this.checked;
            fetch(this.dataset.url, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json', 'Content-Type': 'application/json' } })
            .then(r => r.json()).then(data => {
                Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: data.message || (data.status === 'allowed' ? 'Access granted' : 'Access revoked'), showConfirmButton: false, timer: 3000 });
    ...
    if (cf) cf.addEventListener('change', function() {
        pf.innerHTML = '<option value="">Loading...</option>'; pf.disabled = !this.value;
        if (!this.value) { pf.innerHTML = '<option value="">All Programs</option>'; return; }
        fetch(`{{ url('teacher/todo/colleges') }}/${this.value}/programs`).then(r => r.json()).then(programs => {
        ...
        }).catch(() => { pf.innerHTML = '<option value="">Error loading</option>'; });
    });
    const grantBtn = document.getElementById('grant-selected'), revokeBtn = document.getElementById('revoke-selected');
    if (grantBtn) grantBtn.addEventListener('click', function() {
        ...
        if (!ids.length) { Swal.fire({ icon: 'warning', title: 'No Students Selected', text: 'Please select at least one student.', timer: 2000, showConfirmButton: false }); return; }
        Swal.fire({ title: 'Grant Access?', text: `Grant access to ${ids.length} student(s)?`, icon: 'question', showCancelButton: true, confirmButtonColor: '#48bb78', confirmButtonText: 'Yes, Grant Access' })
        .then(r => { if (r.isConfirmed) { document.getElementById('grant-student-ids').value = JSON.stringify(ids); const f = document.getElementById('grant-form'); f.action = '{{ route("teacher.todo.assignment.grant", $encryptedId) }}'; f.submit(); } });
    });
    if (revokeBtn) revokeBtn.addEventListener('click', function() {
        const ids = Array.from(document.querySelectorAll('.student-checkbox:checked')).map(cb => cb.value);
        if (!ids.length) { Swal.fire({ icon: 'warning', title: 'No Students Selected', text: 'Please select at least one student.', timer: 2000, showConfirmButton: false }); return; }
        Swal.fire({ title: 'Revoke Access?', text: `Revoke access from ${ids.length} student(s)?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#f56565', confirmButtonText: 'Yes, Revoke Access' })
        .then(r => { if (r.isConfirmed) { document.getElementById('revoke-student-ids').value = JSON.stringify(ids); const f = document.getElementById('revoke-form'); f.action = '{{ route("teacher.todo.assignment.revoke", $encryptedId) }}'; f.submit(); } });
    });
}
</script>
@endpush
