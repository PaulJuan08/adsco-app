...
<div class="dashboard-container">
    ...
    <div class="progress-tabs">
        <a href="{{ route('teacher.todo.progress', ['type' => 'quiz'] + request()->except('type', 'page')) }}"
           class="progress-tab {{ $type === 'quiz' ? 'active' : '' }}">
            <i class="fas fa-brain"></i> Quiz Attempts
        </a>
        <a href="{{ route('teacher.todo.progress', ['type' => 'assignment'] + request()->except('type', 'page')) }}"
    ...
    <div class="stats-compact-bar">
        <div class="stat-chip">
            ...
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $totalStudents ?? 0 }}</span>
                <span class="stat-chip-label">Students</span>
            </div>
        </div>

        @if($type === 'quiz')
        <div class="stat-chip">
            <div class="stat-chip-icon"><i class="fas fa-paper-plane"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $totalAttempts ?? 0 }}</span>
                <span class="stat-chip-label">Attempts</span>
            </div>
        </div>
        <div class="stat-chip stat-chip-success">
            <div class="stat-chip-icon"><i class="fas fa-check-circle"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $passedCount ?? 0 }}</span>
                <span class="stat-chip-label">Passed</span>
            </div>
        </div>
        <div class="stat-chip stat-chip-info">
            <div class="stat-chip-icon"><i class="fas fa-chart-line"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $avgScore ? round($avgScore) . '%' : 'N/A' }}</span>
                <span class="stat-chip-label">Avg Score</span>
            </div>
        </div>
        @else
        <div class="stat-chip">
            <div class="stat-chip-icon"><i class="fas fa-upload"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $totalSubmissions ?? 0 }}</span>
                <span class="stat-chip-label">Submissions</span>
            </div>
        </div>
        <div class="stat-chip stat-chip-success">
            <div class="stat-chip-icon"><i class="fas fa-check-circle"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $gradedCount ?? 0 }}</span>
                <span class="stat-chip-label">Graded</span>
            </div>
        </div>
        <div class="stat-chip stat-chip-warning">
            <div class="stat-chip-icon"><i class="fas fa-clock"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $pendingCount ?? 0 }}</span>
                <span class="stat-chip-label">Pending</span>
            </div>
        </div>
        <div class="stat-chip stat-chip-info">
            <div class="stat-chip-icon"><i class="fas fa-star"></i></div>
            <div class="stat-chip-body">
                <span class="stat-chip-value">{{ $avgScore ? round($avgScore) . '%' : 'N/A' }}</span>
                <span class="stat-chip-label">Avg Grade</span>
            </div>
        </div>
        @endif
    </div>
    ...
    <div class="filter-bar">
        <form method="GET" id="filterForm" action="{{ route('teacher.todo.progress') }}">
            ...
            <div class="filter-row">
                ...
                <a href="{{ route('teacher.todo.progress', ['type' => $type]) }}" class="filter-clear-btn">
