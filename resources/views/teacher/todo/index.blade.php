...
<div class="dashboard-container">
    ...
    <div class="stats-grid-compact">
        ...
        <div class="stat-card stat-card-warning clickable-card" onclick="window.location.href='{{ route('teacher.todo.progress', ['type' => 'quiz']) }}'">
            <div class="stat-header">
                <div>
    ...
    <div class="todo-grid">
        ...
        @endphp
        <div class="todo-card" onclick="window.location.href='{{ route('teacher.todo.quiz.show', Crypt::encrypt($quiz->id)) }}'">
            ...
            <div class="todo-card-actions" onclick="event.stopPropagation()">
                <a href="{{ route('teacher.todo.quiz.show', Crypt::encrypt($quiz->id)) }}" 
                ...
                <button type="button" 
                        ...
                        onclick="event.stopPropagation(); showDeleteModal('quiz', '{{ $quiz->title }}', '{{ route('teacher.quizzes.destroy', Crypt::encrypt($quiz->id)) }}')">
                    <i class="fas fa-trash"></i> Delete
                </button>
            </div>
        </div>
    ...
    <div class="todo-grid">
        ...
        @php
            ...
            $avgScore = $assignment->submissions()
                ...
                ->avg('score');
        @endphp
        <div class="todo-card" onclick="window.location.href='{{ route('teacher.todo.assignment.show', Crypt::encrypt($assignment->id)) }}'">
            <div class="todo-card-header">
            ...
            <div class="todo-card-actions" onclick="event.stopPropagation()">
                <a href="{{ route('teacher.todo.assignment.show', Crypt::encrypt($assignment->id)) }}"
                class="btn-xs btn-xs-primary">
                    <i class="fas fa-key"></i> Grant Access
                </a>
                <button onclick="openCrudModal('{{ route('teacher.assignments.edit', Crypt::encrypt($assignment->id)) }}', 'Edit Assignment')"
                class="btn-xs btn-xs-outline">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <a href="{{ route('teacher.todo.progress', ['type' => 'assignment', 'item_id' => $assignment->id]) }}" 
                ...
                <button type="button" 
                        class="btn-xs btn-xs-danger" 
                        onclick="event.stopPropagation(); showDeleteModal('assignment', '{{ $assignment->title }}', '{{ route('teacher.assignments.destroy', Crypt::encrypt($assignment->id)) }}')">
                    <i class="fas fa-trash"></i> Delete
                </button>
            </div>
        </div>
...
<script>
    ...
    function showDeleteModal(type, title, deleteUrl) {
        ...
        itemDisplay.innerHTML = `<i class="fas fa-${type === 'quiz' ? 'brain' : 'file-alt'}"></i> ${title}`;
